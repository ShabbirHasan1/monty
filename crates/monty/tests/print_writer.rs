...
fn print_single_string() {
    ...
    ex.run(vec![], NoLimitTracker::default(), &mut writer).unwrap();
...
fn writer_reuse_accumulates() {
    ...
    ex1.run(vec![], NoLimitTracker::default(), &mut writer).unwrap();

    let ex2 = MontyRun::new("print('second')".to_owned(), "test.py", vec![], vec![]).unwrap();
    ex2.run(vec![], NoLimitTracker::default(), &mut writer).unwrap();
...
";
    ...
    let result = ex.run(vec![], NoLimitTracker::default(), &mut writer);
    assert!(result.is_ok());
}
